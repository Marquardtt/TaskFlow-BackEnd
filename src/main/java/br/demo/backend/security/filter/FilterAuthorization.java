...
public class FilterAuthorization extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (!publicRoute(request)) {
            ...
            Project project = repository.findById(Long.parseLong(projectIdString)).get();

            if (projectIdString != null) {

                Long projectId = Long.parseLong(projectIdString);
                System.out.println(projectId);
                if (!project.getOwner().getUserDetailsEntity().getUsername().equals(user.getUserDetailsEntity().getUsername())) {
